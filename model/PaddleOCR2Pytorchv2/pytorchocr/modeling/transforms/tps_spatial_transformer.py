...
class TPSSpatialTransformer(nn.Module):
    def __init__(self,
        ...
        # self.inverse_kernel = self.inverse_kernel.to("cuda")
        self.padding_matrix = torch.zeros(3, 2)
        # self.padding_matrix = self.padding_matrix.to("cuda")
        self.target_coordinate_repr = target_coordinate_repr
        # self.target_coordinate_repr = self.target_coordinate_repr.to("cuda")
        self.target_control_points = target_control_points

    def forward(self, input, source_control_points):
        self.inverse_kernel = self.inverse_kernel.to(input.device)
        self.padding_matrix = self.padding_matrix.to(input.device)
        self.target_coordinate_repr = self.target_coordinate_repr.to(input.device)
